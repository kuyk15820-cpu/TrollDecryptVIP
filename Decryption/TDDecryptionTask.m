...
- (void)executeWithCompletionHandler:(void (^)(BOOL success, NSURL *outputURL, NSError *error))completionHandler
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        if (self->_destinationPath) {
            NSString *f1x3rFilePath = [self->_destinationPath stringByAppendingPathComponent:@".F1X3R-Decrypt"];
            NSString *f1x3rContent = @"Website: f1x3r.org\nMade with ♡ by F1X3R";
            NSError *writeError = nil;
            [f1x3rContent writeToFile:f1x3rFilePath atomically:YES encoding:NSUTF8StringEncoding error:&writeError];
            if (writeError) {
                NSLog(@"Failed to write .F1X3R-Decrypt file: %@", writeError);
            }
        }

        progress([Localize localizedStringForKey:@"BUILDING_IPA"]);
        NSString *outputIPAName =
            [NSString stringWithFormat:@"%@_%@_F1X3R-Decrypt.ipa",
